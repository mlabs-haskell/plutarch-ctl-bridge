...
-- > psEither = ....
--
newtype FixUpBuilder a = FixUpBuilder (Reader BridgeData a)
  deriving newtype (Functor, Applicative, Monad, MonadReader BridgeData)
...
errorFixUp = do
  ...
  let message =
        "No translation supplied for Haskell type: '"
          ...
          <> inType ^. typePackage
          <> "'!"
  return $ error $ Text.unpack message
...
fixTypeParameters t =
  if "TypeParameters" `Text.isSuffixOf` _typeModule t
    then
      t
        { _typePackage = "" -- Don't suggest any packages
        , _typeModule = "" -- Don't import any modules
        , _typeName = t ^. typeName . to (stripNum . Text.toLower)
        }
    else t
  where
    stripNum v = fromMaybe v (Text.stripSuffix "1" v)
